...
def make_text(chains):
	...
	# your code goes here
	#further study: check if key starts with Capital letter
	#capitalized_key = (list(chains.keys9())).
	#if capitalized, use that key to begin the words = [] list
	
	random_key = choice(list(chains.keys())) # picks a random key & turns it into a list
											 # so we can index it for the 2nd index for 
											 # new key

	while random_key[0][0].islower():
